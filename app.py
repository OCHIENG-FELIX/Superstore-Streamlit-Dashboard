...
with col2:
    ...
    fig2 = px.bar(filtered_df.groupby('Region')['Profit'].sum().reset_index(), x='Region', y='Profit')
    st.plotly_chart(fig2, use_container_width=True)

st.subheader("Sales by Category")
fig3 = px.bar(filtered_df.groupby('Category')['Sales'].sum().reset_index(), x='Category', y='Sales')
